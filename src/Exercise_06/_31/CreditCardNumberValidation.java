...
public class CreditCardNumberValidation {
    ...
    public static boolean isValid(long number ){
        return (getSize( number ) >= 13 && getSize( number ) <= 16 ) &&
                ...
                (numOfDoubleEvenPlace( number ) + sumOfOddPlace( number ))  % 10 == 0;
    }

    /** Get the result from Step 2 */
    public static int numOfDoubleEvenPlace(long number ){
        int sum = 0;
        int count = 0; // To sum just the numbers in the even place
        int oneDigit;
    ...
    public static int sumOfOddPlace(long number){
        int sum = 0;
        int count = 0; // To sum just the numbers in the odd place
        int oneDigit;
